import { episodes } from './url'

export const apiGetEpisodes = (pagesArg, findArg) => {
    ...
    if ( findArg.length > 0 ) {
        findArg.map((item) => {
            ...
            if (item.type === 'episodes') {
                episodesFind = `&${item.type}=${item.value}`;
            }
            return findAll = `${nameFind}${episodesFind}`
        });

    }

    return fetch(`${episodes}${page}${findAll}`)
        .then((response) => {
            if(!response.ok){
